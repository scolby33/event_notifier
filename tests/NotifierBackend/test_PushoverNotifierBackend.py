...
from tests.fixtures import mock_pushover_server, mock_bad_pushover_server, mock_simple_notification, \
    mock_complex_notification, mock_bad_notification
from tests.constants import *


def test_PushoverNotifierBackend_exists():
    assert isinstance(PushoverNotifierBackend(token=TEST_TOKEN, user=TEST_USER), PushoverNotifierBackend)


def test_PushoverNotifierBackend_is_an_ANotifierBackend():
    assert isinstance(PushoverNotifierBackend(token=TEST_TOKEN, user=TEST_USER), ANotifierBackend)


def test_PushoverNotifierBackend_accepts_init_arguments():
    testPushoverNotifierBackend = PushoverNotifierBackend(token=TEST_TOKEN, user=TEST_USER, device=TEST_DEVICE)
...
def test_PushoverNotifierBackend_has_correct_repr():
    testPushoverNotifierBackend = PushoverNotifierBackend(token=TEST_TOKEN, user=TEST_USER, device=TEST_DEVICE)
    assert repr(testPushoverNotifierBackend) == '<PushoverNotifierBackend(token={}, user={}, device={})>'.format(
        TEST_TOKEN, TEST_USER, TEST_DEVICE)
...
@responses.activate
def test_PushoverNotifierBackend_raises_exception_when_a_bad_notification_is_provided(mock_pushover_server,
...
@responses.activate
def test_PushoverNotifierBackend_raises_exception_when_server_rejects_notification(mock_pushover_server,
                                                                                   mock_simple_notification):
    testPushoverNotifierBackend = PushoverNotifierBackend(token=TEST_TOKEN + 'bad', user=TEST_USER, device=TEST_DEVICE)
